...
namespace PN
{
	...
	bool PolishNotation(int lt_position, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		...
		for (int i = lt_position; flag;)
		{
			...
			if (temp.idxTI != -1 && IT::GetEntry(idTable, temp.idxTI).idtype == IT::IDTYPE::F  )
			{
				LT::Entry temp_1;
				temp_1 = LT::GetEntry(lexTable,i);
				temp_1.lexema = '\@';
				char buffer[ID_MAXSIZE];
				int parm_quantity = 0;
				i++;
				for (; LT::GetEntry(lexTable, i).lexema != ')' && LT::GetEntry(lexTable,i).lexema != ';'; i++)
				{

					if (in_ex.code[LT::GetEntry(lexTable, i).lexema] != in_ex.D)
					{
						parm_quantity++;
						temp = LT::GetEntry(lexTable, i);
						literals->push(temp);
				...
				if (LT::GetEntry(lexTable, i).lexema == ';' && parm_quantity == 0)
				{
					break;
				}
			...
			if (temp.lexema == LEX_SAME ||
				temp.lexema == LEX_LESS ||
				temp.lexema == LEX_MORE ||
				temp.lexema == LEX_NOTSAME)
			{
				flag = false; break;
			}
